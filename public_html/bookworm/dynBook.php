...
function renderComments(
    ...
    foreach ($commentsByParent[$parentId] as $c) {
        ...
        $html .= "<p style='white-space: pre-wrap'>" . htmlspecialchars($c['comment_text']) . "</p>";
        $html .= "</div>";
        ...
        if ($c["depth"] < 6 && !empty($user_id)) {
            ...
            $html .= "<form class='replyForm hidden' data-id='" . $c['comment_id'] . "'>";
            $html .= "<input type='hidden' name='bid' value='$book_id'>";
            $html .= "<input type='hidden' name='uid' value='$user_id'>";
            $html .= "<input type='hidden' name='pid' value='" . $c["comment_id"] . "'>";
...
$stmt = $db->prepare(
    'SELECT * FROM usercomments WHERE book_id = ? ORDER BY creation_date ASC'
);
$stmt->bindParam(1, $book_id, PDO::PARAM_INT);
$stmt->execute();
    ...
    <script>
        ...
        document.querySelectorAll('.commentForm').forEach(form => {
            form.addEventListener('submit', function(event) {
                ...
                fetch('/~bdb/bookworm/addCommentBridge.php', {
                        method: 'POST',
