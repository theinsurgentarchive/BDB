...
DROP VIEW IF EXISTS topfivebooks;
...
-- top 5 book (top of the page)
CREATE OR REPLACE VIEW topfivebooks AS 
SELECT B.book_id, B.image_path, B.title, B.author,
AVG(R.rating) AS avg_rating, COUNT(R.rating_id) AS totalratings 
FROM books AS B JOIN ratings AS R ON B.book_id = R.book_id
GROUP BY B.book_id ORDER BY totalratings DESC, avg_rating DESC LIMIT 5;
